...
function MovieSelector({
  ...
  movie: any;
  onClear: () => void;
  onPickMovie: (tmdbId: number) => void;
}) {
  return (
            ...
            <div className="absolute top-14 left-0 right-0 glass-card rounded-xl p-1.5 z-20 shadow-xl animate-fade-in">
              {results.map((m: MovieCompact) => (
                <button
                  type="button"
                  key={m.id || m.tmdb_id}
                  onClick={() => onPickMovie(m.tmdb_id || m.id)}
    ...
    <div className="pt-24 pb-20 px-6 md:px-10 lg:px-20 max-w-[1200px] mx-auto">
      ...
      <div className="flex items-start gap-6 mb-12">
        <MovieSelector
          ...
          onPickMovie={(tmdbId) => void selectMovie(tmdbId, "A")}
        />
        ...
        <MovieSelector
          ...
          onClear={() => {
            setMovieB(null);
            replaceCompareUrl(movieA, null);
          }}
          onPickMovie={(tmdbId) => void selectMovie(tmdbId, "B")}
        />
      </div>
